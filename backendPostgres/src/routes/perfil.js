...
router.get('/perfis', async (req, res) =>{
    try {
        const perfis = await pool.query("Select * from olhabici.perfil");
...
router.delete('/perfis/:id', async (req, res) =>{
    try {
        await pool.query("delete from olhabici.perfil where id_p=$1", [req.params.id])
...
router.put('/perfis/:id', async (req,res) => {
    try {
        ...
        const oldCoord = await pool.query("Select * from coleta_campo.coordenadas where id_p = $1", [id_p])
        //console.log(oldCoord)

        const newCoord = await pool.query(
